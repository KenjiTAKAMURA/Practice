...
x_train = x_train.reshape(-1,1,28,28)  ### 敢えて(-1,28,28)にしてない？
x_test = x_test.reshape(-1,1,28,28)
# ラベルデータをint型にし、one-hot-vectorに変換します
t_train = np.eye(10)[t_train.astype("int")] ### eyeは引数のサイズの単位行列。それを行を指定してスライスする 
...
for i in range(5):
    plt.gray()
    plt.imshow(x_train[i][0]) # reshape(-1,1,28,28)としているため，[0]が必要となる
    plt.show()
...
def im2col(input_data, filter_h, filter_w, stride=1, pad=0):
    ...
    out_h = int(((H + 2*pad) - filter_h)/stride) + 1 #### 問1-1 ####
    out_w = int(((W + 2*pad) - filter_w)/stride) + 1 #### 問1-2 ####
    ...
    col = col.transpose(0, 4, 5, 1, 2, 3).reshape(N*out_h*out_w, -1) #reshapeで2次元にしている。
